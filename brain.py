    ...
    def generate_response(
        ...
        while retries < 2:
            try:
                ...
                print(f"[DEBUG] Pulled from reasoning: {bool(getattr(message, 'reasoning', None) and not message.content)}")
        ...
        messages = [
            {
                "role": "system",
                "content": (
                    "You are a fact extraction system. Extract ALL concrete facts about the user from this conversation. "
                    "Focus on: name, age, gender, location/city, job, hobbies, interests, preferences, goals, fears, favorite things, relationships, family info, pets, health info, etc. "
                    "Only extract information that was EXPLICITLY mentioned by the user. "
                    "Return ONLY valid JSON with this exact format: {\"facts\": [\"fact1\", \"fact2\", ...]}. "
                    "Each fact should be a complete, standalone statement like: \"User's name is John\", \"User is 25 years old\", \"User lives in Beijing\", \"User likes playing guitar\". "
                    "If no user facts found, return {\"facts\": []}."
                )
            },
